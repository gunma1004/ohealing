...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // 100종 풀 고유 해시 계산 (구 + 샵ID 조합)
  const hash = getSeoHash(`${lowerCity}-${lowerDistrict}-${id}-district-shop-seo`);
  const titleModifier = titlePool100[hash % 100];
  const descModifier = descPool100[(hash * 13) % 100];

  // 1. 요청하신 타이틀 형식 적용
  const formattedTitle = `${districtName} 출장 | ${shop.name} - ${locationPrefix} 마사지 ${titleModifier} - 오힐링`;

  // 2. 디스크립션: [구이름] 출장마사지 필수 배치 + 100종 풀 연동
  const formattedDesc = `${districtName} 출장마사지 및 ${shop.name} 코스 요금 안내. ${descModifier}`;
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/${lowerCity}/${lowerDistrict}/shop/${id}`,
    },
    keywords: [
      `${districtName} 출장`,
      `${districtName} 마사지`,
    ...
    openGraph: {
      ...
      description: formattedDesc,
      url: `${SITE_URL}/${lowerCity}/${lowerDistrict}/shop/${id}`,
      locale: "ko_KR",
      type: "website",
      images: [{ url: shop.image, width: 800, height: 600, alt: `${locationPrefix} 마사지 - ${shop.name}` }],
    },
  };
}
...
export default async function DistrictShopDetailPage({ params }: PageProps) {
  ...
  const displayShopTitle = `${locationPrefix} 추천 테라피 - ${shop.name}`;
  ...
  const jsonLd = {
    ...
    "url": `${SITE_URL}/${lowerCity}/${lowerDistrict}/shop/${shop.id}`,
          ...
          <section className="bg-white border border-slate-200 p-6 rounded-3xl space-y-4 shadow-sm">
            <div className="text-center">
              ...
              <h3 className="text-base md:text-xl font-black text-slate-900 mt-1">
                ✨ {locationPrefix} 추천 제휴 샵 (총 {allShopsList.length}곳)
              </h3>
            </div>
              ...
              {allShopsList.map((s) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <Link
                      href={`/${lowerCity}/${lowerDistrict}/shop/${s.id}`}
