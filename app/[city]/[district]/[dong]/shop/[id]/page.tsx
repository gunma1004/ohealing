...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const encodedDong = encodeURIComponent(dongName);
  ...
  // 100종 풀 고유 해시 계산
  const hash = getSeoHash(`${lowerCity}-${lowerDistrict}-${dongName}-${id}-shop-seo`);
  const titleModifier = titlePool100[hash % 100];
  const descModifier = descPool100[(hash * 11) % 100];

  // 1. 요청하신 타이틀 형식 적용
  const formattedTitle = `${dongName} 출장 | ${shop.name} - ${locationKeyword} 마사지 ${titleModifier} - 오힐링`;

  // 2. 디스크립션: [동이름] 출장마사지 필수 포함 + 100종 풀 연동
  const formattedDesc = `${dongName} 출장마사지 및 ${shop.name} 코스 요금 안내. ${descModifier}`;
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/${lowerCity}/${lowerDistrict}/${encodedDong}/shop/${id}`,
    },
    keywords: [
      `${dongName} 출장`,
      `${dongName} 마사지`,
    ...
    openGraph: {
      ...
      description: formattedDesc,
      url: `${SITE_URL}/${lowerCity}/${lowerDistrict}/${encodedDong}/shop/${id}`,
      locale: "ko_KR",
      type: "website",
      images: [{ url: shop.image, width: 800, height: 600, alt: `${locationKeyword} 마사지 - ${shop.name}` }],
    },
  };
}
...
export default async function DongShopDetailPage({ params }: PageProps) {
  ...
  const displayShopTitle = `${fullLocation} 추천 테라피 - ${shop.name}`;
  ...
  const jsonLd = {
    ...
    "url": `${SITE_URL}/${lowerCity}/${lowerDistrict}/${encodedDong}/shop/${shop.id}`,
    ...
    <div className="bg-slate-50 text-slate-800 min-h-screen flex flex-col font-sans pb-28">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 px-4 py-3 shadow-sm">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <Link
            href={`/${lowerCity}/${lowerDistrict}/${encodedDong}`}
          ...
          <section className="bg-white border border-slate-200 p-6 rounded-3xl space-y-4 shadow-sm">
            <div className="text-center">
              ...
              <h3 className="text-base md:text-xl font-black text-slate-900 mt-1">
                ✨ {fullLocation} 추천 제휴 샵 (총 {allShopsList.length}곳)
              </h3>
            </div>
              ...
              {allShopsList.map((s) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <Link
                      href={`/${lowerCity}/${lowerDistrict}/${encodedDong}/shop/${s.id}`}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-t border-slate-200 p-3 md:p-4 shadow-lg">
        <div className="max-w-4xl mx-auto grid grid-cols-2 gap-3">
          <a
            href={`tel:${shop.phone}`}
            className="flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-black py-3.5 rounded-2xl text-xs md:text-sm shadow-sm transition-all active:scale-98"
          >
            📞 전화예약 ({shop.phone})
          </a>
          <a
            href={`sms:${shop.phone}`}
            className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-900 font-black py-3.5 rounded-2xl text-xs md:text-sm transition-all active:scale-98"
          >
            💬 문자상담
          </a>
        </div>
