...
export const metadata: Metadata = {
  title: `인천 마사지 | 인천 구·군 전체 지역별 제휴 힐링 테라피 안내 - ${SITE_NAME}`,
  description: "인천 출장마사지 및 프리미엄 홈케어 테라피 제휴 안내. 인천 전 지역 세부 동·면별 검증된 바디케어 정보를 오힐링에서 편리하게 확인하세요.",
  alternates: {
  ...
  openGraph: {
    title: `인천 마사지 | 인천 구·군 전체 지역별 제휴 힐링 테라피 - ${SITE_NAME}`,
    description: "인천 출장마사지 및 프리미엄 바디케어 제휴 정보 안내. 인천 전 지역 세부 매장 정보를 한눈에 비교해 보세요.",
...
export default function IncheonRegionPage() {
  const incheonDistricts = regionData.incheon.districts;
    ...
    <main className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <nav className="bg-white border-b border-slate-200 py-3 px-4 text-xs text-slate-500">
        <div className="max-w-6xl mx-auto flex items-center gap-2">
          ...
          <span>&gt;</span>
          <span className="text-slate-700 font-bold">인천 지역 안내</span>
        </div>
      </nav>
      ...
      <footer className="bg-white border-t border-slate-200 py-8 text-center text-xs text-slate-400 mt-20">
        ...
        <p className="mt-1">도메인: https://ohealing.netlify.app/incheon</p>
      </footer>
    </main>
  );
}