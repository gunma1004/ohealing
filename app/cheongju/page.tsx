...
export const metadata: Metadata = {
  title: `청주 마사지 | 청주 4개 구 전체 지역별 제휴 힐링 테라피 안내 - ${SITE_NAME}`,
  description: "청주 출장마사지 및 프리미엄 홈케어 테라피 제휴 안내. 청주 전 지역(상당구, 서원구, 흥덕구, 청원구) 세부 동·읍별 검증된 바디케어 정보를 오힐링에서 편리하게 확인하세요.",
  alternates: {
    canonical: `${SITE_URL}/cheongju`,
  },
  keywords: [
    "청주 마사지",
    "청주 출장마사지",
  ...
  openGraph: {
    title: `청주 마사지 | 청주 4개 구 전체 지역별 제휴 힐링 테라피 - ${SITE_NAME}`,
    description: "청주 출장마사지 및 프리미엄 바디케어 제휴 정보 안내. 청주 전 지역 세부 매장 정보를 한눈에 비교해 보세요.",
    url: `${SITE_URL}/cheongju`,
...
export default function CheongjuRegionPage() {
  const cheongjuDistricts = regionData.cheongju.districts;
    ...
    <main className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <nav className="bg-white border-b border-slate-200 py-3 px-4 text-xs text-slate-500">
        <div className="max-w-6xl mx-auto flex items-center gap-2">
          ...
          <span>&gt;</span>
          <span className="text-slate-700 font-bold">청주 지역 안내</span>
        </div>
      </nav>
      ...
      <section className="max-w-6xl mx-auto py-10 px-4">
        <div className="mb-8">
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 mb-2">
            청주 4개 구 전체 지역별 프리미엄 힐링 마사지 테라피
          </h1>
          <p className="text-slate-600 text-sm md:text-base">
            청주 전 지역(4개 구) 세부 동·읍별 제휴 정보를 편리하게 확인하세요.
          </p>
        </div>

        <div className="space-y-6">
          {Object.entries(cheongjuDistricts).map(([districtKey, districtVal]) => (
            <div key={districtKey} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {districtVal.dongs.map((dong, idx) => (
                  <Link
                    key={idx}
                    href={`/cheongju/${districtKey}/${encodeURIComponent(dong)}`}
      ...
      <footer className="bg-white border-t border-slate-200 py-8 text-center text-xs text-slate-400 mt-20">
        ...
        <p className="mt-1">도메인: https://ohealing.netlify.app/cheongju</p>
      </footer>
    </main>
  );
}